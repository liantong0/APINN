function gradients = compute_gradients_tdoa_adaptive(net, X_norm, X_raw, Y_pred, Y_true, activations, ...
                                            ...
                                            lambda_tdoa, lambda_camera, lambda_detector, ...
                                            lambda_depth, lambda_consistency, optical_attenuation)
% Compute gradients for the adaptive-weight APINN objective.
...
d_true_batch = Y_true(:,1);

sample_weight_dist = ones(batch_size,1);
sample_weight_angle = ones(batch_size,1);

for i = 1:batch_size
    if d_true_batch(i) <= 2
        sample_weight_dist(i) = 2.8;
        sample_weight_angle(i) = 1.00;
    elseif d_true_batch(i) <= 5
        sample_weight_dist(i) = 2.0;
        sample_weight_angle(i) = 1.00;
    elseif d_true_batch(i) <= 10
        sample_weight_dist(i) = 1.4;
        sample_weight_angle(i) = 1.00;
    else
        sample_weight_dist(i) = 1.0;
        sample_weight_angle(i) = 1.0;
    end
end

ow = train_params.output_weights;
angle_data_scale = 0.12;

eps_d = 1e-3;
eps_ang = deg2rad(3.0);

err_d = Y_pred(:,1) - Y_true(:,1);
err_psi = atan2(sin(Y_pred(:,2) - Y_true(:,2)), cos(Y_pred(:,2) - Y_true(:,2)));
err_theta = atan2(sin(Y_pred(:,3) - Y_true(:,3)), cos(Y_pred(:,3) - Y_true(:,3)));

grad_d = err_d ./ sqrt(err_d.^2 + eps_d^2);


grad_psi = err_psi ./ sqrt(err_psi.^2 + eps_ang^2);
grad_theta = err_theta ./ sqrt(err_theta.^2 + eps_ang^2);

dL_dy(:,1) = dL_dy(:,1) + 2 * ow(1) * sample_weight_dist .* grad_d / batch_size;
dL_dy(:,2) = dL_dy(:,2) + angle_data_scale * ow(2) * sample_weight_angle .* grad_psi / batch_size;
dL_dy(:,3) = dL_dy(:,3) + angle_data_scale * ow(3) * sample_weight_angle .* grad_theta / batch_size;
    ...
    for i = 1:batch_size
        ...
        robust_residuals = residuals ./ sqrt(residuals.^2 + eps_t^2);
        grad_tdoa = (lambda_tdoa(i) * S_TDOA) * J_pos' * grad_dist' * (2 * robust_residuals) / batch_size;
    ...
    for i = 1:batch_size
        ...
        g_dep = depth_residual / sqrt(depth_residual^2 + eps_dep^2);
        
        dL_dy(i, 1) = dL_dy(i, 1) + 2 * lambda_depth(i) * ...
                      g_dep * sin(theta_pred(i)) / batch_size;
        dL_dy(i, 3) = dL_dy(i, 3) + 2 * lambda_depth(i) * ...
                      g_dep * d_pred(i) * cos(theta_pred(i)) / batch_size;
    end
