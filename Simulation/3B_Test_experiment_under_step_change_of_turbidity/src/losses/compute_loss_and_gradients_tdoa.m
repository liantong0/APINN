function [loss, gradients] = compute_loss_and_gradients_tdoa(net, X_norm, X_raw, ...
                                                        ...
                                                        params, train_params)
% Compute adaptive APINN loss terms and backpropagation gradients.
    ...
    for i = 1:batch_size
        if is_near(i)
            ...
            adaptive_lambda_tdoa(i) = train_params.lambda_tdoa * 0.15;
            adaptive_lambda_depth(i) = train_params.lambda_depth * 6.0;
        else
...
eps_d = 1e-3;
eps_ang = deg2rad(3.0);   % ?2.5~4.0


rho_d = sqrt(err_d.^2 + eps_d^2);

rho_psi = sqrt(err_psi.^2 + eps_ang^2) - eps_ang;
rho_theta = sqrt(err_theta.^2 + eps_ang^2) - eps_ang;

ow = train_params.output_weights;
angle_data_scale = 0.12;   % ?0.08~0.2

loss_data = sum( ...
    sample_weight_dist .* (ow(1) * rho_d.^2) + ...
    sample_weight_angle .* (angle_data_scale * ow(2) * rho_psi + ...
                            angle_data_scale * ow(3) * rho_theta) ...
    ) / batch_size;
    ...
    for i = 1:batch_size
        ...
        rho2 = sqrt(residual_2^2 + eps_t^2);
        rho3 = sqrt(residual_3^2 + eps_t^2);
        rho4 = sqrt(residual_4^2 + eps_t^2);
    
        loss_tdoa = loss_tdoa + (adaptive_lambda_tdoa(i) * S_TDOA) * ...
                    (rho2^2 + rho3^2 + rho4^2);
    end
    ...
    for i = 1:batch_size
        ...
        rho_dep = sqrt(depth_residual^2 + eps_dep^2);
        
        loss_depth = loss_depth + adaptive_lambda_depth(i) * rho_dep^2;
    end
