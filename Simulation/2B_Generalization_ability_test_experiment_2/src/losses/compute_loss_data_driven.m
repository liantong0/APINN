...
% Compute data-driven loss and gradients with regularization and clipping.
    ...
    grad_clip = 0.5;
    for i = 1:num_layers
        gradients.W{i} = max(min(gradients.W{i}, grad_clip), -grad_clip);
        gradients.b{i} = max(min(gradients.b{i}, grad_clip), -grad_clip);
    end
end
