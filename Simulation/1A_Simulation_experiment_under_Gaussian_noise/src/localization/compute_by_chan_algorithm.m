...
% Estimate distance and angles using a hybrid strategy with Chan, camera, and detector measurements.

    num_samples = size(X_raw, 1);
    Y_result = zeros(num_samples, 3);
    distance_threshold = 2.0;  % Near-range threshold
    
    near_count = 0;
    far_count = 0;
    
    for i = 1:num_samples
        % First estimate distance roughly from depth and camera angle
        v = X_raw(i, 5);
        ...
        % Select method based on the rough distance estimate
        if d_rough <= distance_threshold
            ...
            % Camera-based distance estimate
