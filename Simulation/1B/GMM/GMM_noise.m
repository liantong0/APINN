...
%% Parameter notes
% layers = [11, 32, 64, 64, 32, 3];  % Input dimension = 11:
% % 3 TDOA + u, v + U_I~U_IV + depth + turbidity
%
% params.c_water = 1500;
% params.f_U = 800;
% params.f_V = 800;
...
% params.l2 = 20;
%
% % TDOA hydrophone array: reference station + 3 measurement stations
...
%
% % Unified noise setting based on SNR (dB)
% params.SNR = 55;
...
%
% train_params.output_weights = [9.0, 14.0, 1.0];
%
% % Fixed weights
...
%
% params.noise.tdoa.type     = 'gmm';
% params.noise.camera.type   = 'gmm';
% params.noise.detector.type = 'gmm';
% params.noise.depth.type    = 'gmm';
%
% % Turbidity parameters
% params.turbidity = 10;  % NTU
% % Typical range: 0-5 NTU (clear), 5-15 NTU (moderate), >15 NTU (turbid)
%
% params.turbidity_coefficient = 0.2;
% % Optical attenuation coefficient:
% % c = c_base + k_turb * turbidity

%% Load the fully reproducible state saved by Script 3
load('Alpha_Distance_Repro_State.mat', ...
...
% Restore the RNG state before training
rng(rng_state_at_training_start);

fprintf('Loaded reproducible training state file: Alpha_Distance_Repro_State.mat\n');
...
loss_history_pinn = struct('total', [], 'data', [], 'physics', [], ...
                           'tdoa', [], 'camera', [], 'detector', [], ...
                           'depth', [], 'consistency', []);
...
for epoch = 1:train_params.num_epochs
    ...
    epoch_loss = struct('total', 0, 'data', 0, 'physics', 0, ...
                        'tdoa', 0, 'camera', 0, 'detector', 0, ...
                        'depth', 0, 'consistency', 0);
    ...
    for batch = 1:num_batches
        ...
        epoch_loss.physics     = epoch_loss.physics + loss.physics;
    ...
    epoch_loss.physics     = epoch_loss.physics / num_batches;
    ...
    loss_history_pinn.physics(epoch)     = epoch_loss.physics;
    ...
    if mod(epoch, 500) == 0 || epoch == 1
        ...
        fprintf('       Total: %.6f | Data: %.6f | Physics: %.6f\n', ...
                epoch_loss.total, epoch_loss.data, epoch_loss.physics);
    end
end
...
loss_history_pinn_fixed = struct('total', [], 'data', [], 'physics', [], ...
                                 'tdoa', [], 'camera', [], 'detector', [], ...
                                 'depth', [], 'consistency', []);
...
for epoch = 1:train_params.num_epochs
    ...
    loss_history_pinn_fixed.physics(epoch)     = epoch_loss.physics;
...
fprintf('Start training pure data-driven neural network\n');
...
weight_decay = 0.25;  % Keep consistent with Script 3
...
physics_loss_total_apinn = loss_history_pinn.physics;
physics_loss_total_fixed = loss_history_pinn_fixed.physics;
...
%% Figure 2: MAE comparison + improvement percentage
...
%% Figure 3: Localization error vs. distance for four methods
...
ax = axes(fig4); hold(ax, 'on'); box(ax, 'on');
...
for i = 1:3
   plot(ax, p_chan(i),  percentiles(i) / 100, markers{i}, 'MarkerSize', marker_sizes(i), ...
       'MarkerFaceColor', color_red, 'MarkerEdgeColor', 'w', 'LineWidth', 1.5, 'HandleVisibility', 'off');
   plot(ax, p_data(i),  percentiles(i) / 100, markers{i}, 'MarkerSize', marker_sizes(i), ...
       'MarkerFaceColor', color_green, 'MarkerEdgeColor', 'w', 'LineWidth', 1.5, 'HandleVisibility', 'off');
   plot(ax, p_fixed(i), percentiles(i) / 100, markers{i}, 'MarkerSize', marker_sizes(i), ...
       'MarkerFaceColor', color_purple, 'MarkerEdgeColor', 'w', 'LineWidth', 1.5, 'HandleVisibility', 'off');
   plot(ax, p_pinn(i),  percentiles(i) / 100, markers{i}, 'MarkerSize', marker_sizes(i), ...
       'MarkerFaceColor', color_blue, 'MarkerEdgeColor', 'w', 'LineWidth', 1.5, 'HandleVisibility', 'off');
end
