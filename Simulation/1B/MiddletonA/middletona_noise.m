...
%% Parameter settings
% layers = [11, 32, 64, 64, 32, 3];  % Input dimension is 11 (3 TDOAs + u, v, U_I~U_IV, depth, turbidity)
% 
% params.c_water = 1700;
% params.f_U = 800;
% params.f_V = 800;
...
% params.l2 = 20;
% 
% % TDOA hydrophone array configuration (quaternary array, reference station + 3 measurement stations)
...
% 
% train_params.output_weights = [9.0, 15.0, 1.0];
% 
% % Fixed weights
% train_params.lambda_tdoa = 3.0;      % TDOA constraint weight
% train_params.lambda_depth = 5.0;     % Depth constraint weight
% train_params.lambda_consistency = 3.0;  % Consistency constraint weight
% train_params.lambda_camera = 5;     % Camera constraint weight
% train_params.lambda_detector = 2;   % Detector constraint weight
% train_params.tdoa_loss_scale = 1e6;
% 
% % Noise type switch: use GMM 
% params.noise.gmm.eps   = 0.1;   % Impulsive noise probability
% params.noise.gmm.kappa = 10;    % Variance amplification factor of impulsive component 
% 
...
% params.noise.midA.A     = 0.1;    % Impulsive index A 
% params.noise.midA.Gamma = 0.01;   % Gaussian-to-impulsive power ratio Γ 
% params.noise.midA.Mmax  = 40;     % Number of truncation terms
% 
% params.noise.tdoa.type     = 'middletona';
% params.noise.camera.type   = 'middletona';
% params.noise.detector.type = 'middletona';
% params.noise.depth.type    = 'middletona';
% 
% % Turbidity parameters 
% params.turbidity = 10;  % NTU 
% 
% params.turbidity_coefficient = 0.2; 
% % Optical attenuation coefficient c = c_base + k_turb * turbidity

%% Load reproducible experiment state
load('MiddletonA_Distance_Repro_State.mat');

fprintf('Loaded reproducible experiment state file: MiddletonA_Distance_Repro_State.mat\n');

rng(rng_state_at_training_start);

%% Normalize data
fprintf('Normalizing training data...\n');
...
for epoch = 1:train_params.num_epochs
    ...
    for batch = 1:num_batches
        ...
        [net_pinn, adam_state_pinn] = adam_update( ...
            net_pinn, gradients, adam_state_pinn, current_lr_pinn, ...
    ...
    if mod(epoch, 500) == 0 || epoch == 1
        physics_loss = epoch_loss.tdoa + epoch_loss.camera + epoch_loss.detector + ...
...
for epoch = 1:train_params.num_epochs
    ...
    for batch = 1:num_batches
        ...
        [net_pinn_fixed, adam_state_pinn_fixed] = adam_update( ...
            net_pinn_fixed, gradients, adam_state_pinn_fixed, current_lr_pinn_fixed, ...
            train_params.beta1, train_params.beta2, train_params.epsilon, epoch);
...
fprintf('Fixed-PINN training completed!\n\n');

%% Train Data-driven NN
fprintf('\nStart training Data-driven NN\n');
...
for epoch = 1:train_params.num_epochs
    ...
    for batch = 1:num_batches
        ...
        [loss, gradients] = compute_loss_data_driven( ...
            net_data, X_batch, Y_pred, Y_batch, activations, train_params, weight_decay);

        [net_data, adam_state_data] = adam_update( ...
            net_data, gradients, adam_state_data, current_lr_data, ...
            train_params.beta1, train_params.beta2, train_params.epsilon, epoch);
...
fprintf('Data-driven NN training completed!\n\n');
...
%% Figure 1: PINN loss curves + 3D position error evolution
fig1 = figure('Position', [100, 100, 1400, 600], 'Color', 'w');

% Subplot 1: Loss comparison of three methods (including Data-driven NN)
subplot(1,2,1);
...
% Subplot 2: 3D position error evolution of three methods
subplot(1,2,2);
hold on; box on; grid on;
...
text(train_params.num_epochs*0.55, max([position_error_history_pinn, position_error_history_pinn_fixed, position_error_history_data])*0.75, ...
    sprintf('Final Error:\nAPINN: %.3f m\nFixed-PINN: %.3f m\nData-NN: %.3f m', ...
...
print(fig1, 'Fig1_Combined_PINN_Comparison.png', '-dpng', '-r300');
...
print(fig2, 'Fig2_MAE_Comparison_and_Improvement.png', '-dpng', '-r300');

%% Figure 3: Localization error versus distance for four methods
...
print(fig3, 'Fig3_Error_vs_Distance_FourMethods.png', '-dpng', '-r300');

%% Figure 4: Error CDF for four methods
fig4 = figure('Position', [100, 100, 1000, 650], 'Color', 'w', 'Renderer', 'painters');
ax = axes(fig4); 
hold(ax, 'on'); 
box(ax, 'on');

e_chan  = error_chan(:);       
e_chan  = e_chan(isfinite(e_chan)  & e_chan >= 0);
e_data  = error_data(:);       
e_data  = e_data(isfinite(e_data)  & e_data >= 0);
e_fixed = error_pinn_fixed(:); 
e_fixed = e_fixed(isfinite(e_fixed) & e_fixed >= 0);
e_pinn  = error_pinn(:);       
e_pinn  = e_pinn(isfinite(e_pinn)  & e_pinn >= 0);
...
h_chan  = plot(ax, x_chan,  f_chan,  'LineWidth', 3.0, 'Color', color_red,    'DisplayName', 'Analytical');
...
print(fig4, 'Fig4_Error_CDF_FourMethods.png', '-dpng', '-r300');
